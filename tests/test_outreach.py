...
def test_add_and_list_roundtrip(tmp_path):
    ...
    outreach.add_contact("Varun", "ZoomInfo", role_context="Agentic Tools PM",
                         message_type="hm-message", on_date="2026-07-07", path=log)
    outreach.add_contact("Raveena", "WHOOP", on_date="2026-07-08", path=log)

    records = outreach.list_contacts(path=log)
    assert [r["name"] for r in records] == ["Varun", "Raveena"]  # sorted oldest first
...
def test_add_requires_name_and_company(tmp_path):
    ...
    with pytest.raises(ValueError):
        outreach.add_contact("Varun", "   ", path=log)


def test_add_rejects_unknown_message_type(tmp_path):
    with pytest.raises(ValueError):
        outreach.add_contact("Varun", "ZoomInfo", message_type="dm", path=tmp_path / "o.jsonl")


def test_remove_contact_is_case_insensitive(tmp_path):
    log = tmp_path / "outreach.jsonl"
    outreach.add_contact("Devin Hua", "Axon", on_date="2026-07-08", path=log)
    outreach.add_contact("Spiro Sacre", "Axon", on_date="2026-07-08", path=log)

    removed = outreach.remove_contact("devin hua", "AXON", path=log)
    assert [r["name"] for r in removed] == ["Devin Hua"]
    assert [r["name"] for r in outreach.list_contacts(path=log)] == ["Spiro Sacre"]
