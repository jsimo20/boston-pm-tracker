...
if (-not (Test-Path $exe)) {
    Write-Error "job-finder.exe not found at $exe — create the venv and 'pip install -e .' first (SETUP.md §2)"
    exit 1
}
...
Register-ScheduledTask -TaskName "job-finder weekly" -Action $action `
    -Trigger $trigger -Settings $settings -Description `
    "Weekly job-finder pipeline: collect, extract, score, digest, email." -Force | Out-Null

Write-Host "Registered 'job-finder weekly' (Mondays 09:00, runs at next boot if missed)."
Write-Host "It needs ANTHROPIC_API_KEY, GMAIL_USER, GMAIL_APP_PASSWORD in $repo\.env"
